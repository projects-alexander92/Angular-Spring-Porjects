...
public class UserService implements UserDetailsService {
    ...
    public void register(RegistrationModel registrationModel) {
        ...
        user.setFaction(registrationModel.getFaction());
        Authority authority = this.authorityRepository.findByAuthority("ROLE_USER");
        user.getAuthorities().add(authority);
        this.userRepository.save(user);
    }

    public Faction getFactionByUsername(String username) {
       return this.userRepository.getFactionByUsername(username);
    }
}
